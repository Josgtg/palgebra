...
impl Scanner {
    ...
    fn match_token(&mut self) {
        match self.advance() {
            ...
            't' => {
                if self.match_str("rue") {
                    self.tokens.push(Token::True)
                } else {
                    self.tokens.push(Token::Sentence('t'));
                }
            }
            '0' => self.tokens.push(Token::False),
            'f' => {
                if self.match_str("alse") {
                    self.tokens.push(Token::False)
                } else {
                    self.tokens.push(Token::Sentence('f'));
                }
            }
            _ => {
    ...
    fn advance_until(&mut self, to_stop: char) {
        while !self.is_at_end() && !self.match_char(to_stop) {
            self.advance();
        }
    }
